        ...
        <table class="min-w-full border border-gray-300 text-sm text-center">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-4 py-2 border">TGL DITAMBAHKAN</th>
                ...
                <tr>
                    ...
                    <td class="border px-4 py-2">
                        <div class="flex flex-col items-center gap-1">
                            <button onclick="openEditMobilModal('{{ $mobil->kode_mobil }}')" class="bg-yellow-400 text-white px-3 py-1 rounded hover:bg-yellow-500 transition-colors w-full">
                                Edit
                            </button>
    ...
    <div class="relative w-full max-w-md mx-auto mt-20">
        <div class="relative bg-white rounded-lg shadow">
            <div class="flex items-start justify-between p-4 border-b rounded-t">
                ...
                <button type="button" onclick="closeEditMobilModal()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center">
                    &times;
                </button>
            </div>
...
<script>
    ...
    function closeEditMobilModal() {
        document.getElementById('editMobilModal').classList.add('hidden');
        document.body.classList.remove('overflow-hidden');
    }

    // Edit Modal Function - COMPLETELY REWRITTEN
    async function openEditMobilModal(kode_mobil) {
        try {
            // Show modal immediately with loading state
            const editModal = document.getElementById('editMobilModal');
            ...
            console.log('Fetching data for kode_mobil:', kode_mobil);
            
            // Fetch data from server
            const response = await fetch(`/mobiladmin/${kode_mobil}/edit`, {
                method: 'GET',
                headers: {
            ...
            // Build the form HTML
            modalContent.innerHTML = `
                <form id="editForm" action="/mobiladmin/${kode_mobil}" method="POST" enctype="multipart/form-data" class="space-y-3">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex justify-end gap-3 pt-3 border-t">
                        <button type="button" onclick="closeEditMobilModal()" class="px-4 py-1.5 text-sm rounded-md bg-gray-300 text-gray-700 font-medium hover:bg-gray-400">
                            Tutup
                        </button>
            ...
            console.error('Error in openEditMobilModal:', error);
            ...
            modalContent.innerHTML = `
                <div class="text-center py-8">
                    ...
                    <button onclick="closeEditMobilModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
                        Tutup
                    </button>
                </div>
            `;
    ...
    window.onclick = function(event) {
        const tambahModal = document.getElementById('tambahMobilModal');
        const editModal = document.getElementById('editMobilModal');
